...
//#define DEBUG_DRAW_MAX_POINTS 8192 // default
#define DEBUG_DRAW_MAX_POINTS 81920
#endif // DEBUG_DRAW_MAX_POINTS

#ifndef DEBUG_DRAW_MAX_LINES
//#define DEBUG_DRAW_MAX_LINES 32768 //default
#define DEBUG_DRAW_MAX_LINES 327680
...
//#define DEBUG_DRAW_VERTEX_BUFFER_SIZE 4096 // default
#define DEBUG_DRAW_VERTEX_BUFFER_SIZE 40960
...
void xySquareGrid(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx,)
		float mins,
		float maxs,
		float z,
		float step,
...
void yzSquareGrid(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx,)
		float mins,
		float maxs,
		float x,
		float step,
...
void xySquareGrid(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx,) const float mins, const float maxs, const float z,
                  const float step, ddVec3_In color, const int durationMillis, const bool depthEnabled)
{
    if (!isInitialized(DD_EXPLICIT_CONTEXT_ONLY(ctx)))
    {
...
void yzSquareGrid(DD_EXPLICIT_CONTEXT_ONLY(ContextHandle ctx,) const float mins, const float maxs, const float x,
                  const float step, ddVec3_In color, const int durationMillis, const bool depthEnabled)
{
    if (!isInitialized(DD_EXPLICIT_CONTEXT_ONLY(ctx)))
    {
    ...
    for (float i = mins; i <= maxs; i += step)
    {
        // Horizontal line (along the Z)
        vecSet(from, x, i, mins);
        vecSet(to,   x, i, maxs);
        line(DD_EXPLICIT_CONTEXT_ONLY(ctx,) from, to, color, durationMillis, depthEnabled);

        // Vertical line (along the Y)
        vecSet(from, x, mins, i);
        vecSet(to,   x, maxs, i);
        line(DD_EXPLICIT_CONTEXT_ONLY(ctx,) from, to, color, durationMillis, depthEnabled);
    }
}
